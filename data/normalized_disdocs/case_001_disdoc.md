...
- **Semantic Metrics**: BERTScore, BLEURT, NUBIA.
- **Evaluation Granularity**: Metrics are reported for Top-1 and Top-5 recommendations.
...
- Comparison across three settings: Zero-shot, Fine-tuned, and Multi-task.
- Comparison across model generations (GPT-3 vs. GPT-3.5).
...
- Comparison of generated text vs. ground truth using the metric suite (BLEU, ROUGE, etc.).
- Result: Davinci-002 (GPT-3.5) provided $\ge 15.38\%$ gain for root cause and $\ge 11.9\%$ gain for mitigation tasks over GPT-3.

**ii. Human evaluation**
- Interviews with incident owners to validate real-world utility.
