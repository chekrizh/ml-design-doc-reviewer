...
**i. Origin**
Dropbox manages payment processing for millions of customers. When recurring payments (monthly or yearly) fail, customers enter a "renewal failure" state. Historically, Dropbox used a static set of ~several hardcoded rules to determine retry schedules (e.g., charging every several days for a maximum of several days). If all attempts fail, the account is downgraded to a free Basic account.
...
- **Operational Overhead**: The previous manual process of segmenting Scale details are not documented yet.
...
**iv. Previous work**
- **Rule-based System**: A set of ~several hardcoded rules based on human intuition and domain knowledge.
...
- **Patterns**: Recurring billing cycles (monthly/yearly) with retry windows (e.g., several several or several-day windows).
